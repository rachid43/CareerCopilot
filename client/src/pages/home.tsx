...
import { SubscriptionGate, SubscriptionStatus, UsageLimit } from "@/components/subscription-gate";
...
export default function Home() {
  ...
  const { hasAccess: hasJobTracker } = useFeatureAccess('jobTrackerFull');
  const { hasAccess: hasChatAccess } = useFeatureAccess('aiMentorChat');
  const { hasAccess: hasTextInterviews } = useFeatureAccess('textInterviews');
  const { hasAccess: hasAvatarInterviews } = useFeatureAccess('avatarInterviews');
  const { allowed: canCreateCV, remaining: cvRemaining } = useUsageLimit('maxCvGenerations', 3); // Mock usage
  const { allowed: canInterview, remaining: interviewRemaining } = useUsageLimit('maxInterviewSessions', 2); // Mock usage
